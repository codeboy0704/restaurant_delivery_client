                        ...
                        {menu_list.map((el, i) => {
                            return (
                                <div onClick={()=> setCategory(prev => prev === el.menu_name ? "All": el.menu_name)} key={i}>
                                    <img className={category === el.menu_name ? "select w-[12px] min-w-[80px] sm:min-w-[90px] md:min-w-[120px] pointer rounded-[50%]" : "w-[40px] min-w-[80px] sm:min-w-[90px] md:min-w-[120px] pointer rounded-xl"} src={el.menu_image} alt="" />
